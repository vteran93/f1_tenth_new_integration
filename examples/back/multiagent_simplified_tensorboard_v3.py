...
class MultiAgentF110(MultiAgentEnv):
    ...
    def step(self, action_dict):
        actions = np.array([action_dict[agent] for agent in self.agents], dtype=np.float32)
        print(f"Step actions: {actions}")  # Debug: Print actions
        ...
        for i, agent_id in enumerate(self.agents):
            ...
            print(f"Agent {agent_id} reward: {rewards[i]}, Env state: poses_x={self.env.poses_x}, poses_y={self.env.poses_y}, collisions={self.env.collisions}")  # Debug: Print rewards and state
    ...
    def logger_creator(config):
        ...
        class CustomLogger(LoggerCallback):
            ...
            def on_result(self, result):
                ...
                # Log per-agent rewards to TensorBoard
                for agent_id in ['agent_0', 'agent_1']:
                    if f'policy_{agent_id}_reward_mean' in filtered_result:
                        self.tb_writer.add_scalar(f'reward/{agent_id}', 
                                                 filtered_result[f'policy_{agent_id}_reward_mean'], 
                                                 self.iteration)
                # Log to JSON
...
def setup_training(config):
    ...
    while True:
        ...
        # Calculate and log per-agent and average episode rewards
        agent_rewards = {agent_id: results[agent_id].get(best_metric, 0.0) for agent_id in algorithms}
        avg_episode_reward = np.mean(list(agent_rewards.values()))
        
        if timesteps_total % save_every == 0:
            print(f"Timesteps: {timesteps_total}, Per-Agent Rewards: {agent_rewards}, Avg Episode Reward: {avg_episode_reward}")
            for agent_id, algo in algorithms.items():
