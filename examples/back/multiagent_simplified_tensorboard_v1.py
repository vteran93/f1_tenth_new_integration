...
class MultiAgentF110(MultiAgentEnv):
    ...
    def __init__(self, env_config=None):
        ...
        if reward_strategy_name == "combined":
            for strategy_name in ["progress", "speed_track", "crosstrack", "talearning"]:
    ...
    def step(self, action_dict):
        #print("Stepping with actions:", action_dict) #PEPE
        
        actions = np.array([action_dict[agent] for agent in self.agents])
        # Update actions for TALearningReward
        for strategy in self.reward_strategies:
            if isinstance(strategy, TALearningReward):
                strategy.update_actions(actions)
        
        obs, _, terminated, truncated, info = self.env.step(actions)

        # Calculate rewards using all strategies, passing obs
        rewards = np.zeros(self.env.num_agents)
        for strategy in self.reward_strategies:
            rewards += np.array(strategy.compute_rewards(self.env, obs))
