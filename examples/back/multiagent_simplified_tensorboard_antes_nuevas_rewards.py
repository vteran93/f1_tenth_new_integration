...
try:
    ...
    raise ImportError("Cannot import 'rewards'. Make sure 'rewards.py' exists in the same directory or is in your PYTHONPATH.")
try:
    from back.algorithms import get_algorithm
except ImportError:
    raise ImportError("Cannot import 'algorithms'. Make sure 'algorithms.py' exists in the same directory or is in your PYTHONPATH.")
...
def setup_policies_and_config(config):
    ...
    algo_config = {"env_config": env_config}  # Pass env_config to algorithm
    algo = get_algorithm({"algorithm": config["algorithm"], "env_config": env_config}, env_name, policies)
    algo.setup_config()
