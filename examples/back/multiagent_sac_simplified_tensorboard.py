...
#Por recompensas / rewards
from rewards.rewards import ProgressReward, SpeedTrackReward
...
class MultiAgentF110(MultiAgentEnv):
    ...
    def __init__(self, env_config=None):
        ...
        if reward_strategy_name == "progress":
            self.reward_strategy = ProgressReward(self.env.num_agents)
        elif reward_strategy_name == "speed_track":
            self.reward_strategy = SpeedTrackReward(self.env.num_agents)
        else:
    ...
    def reset(self, *, seed=None, options=None):
        ...
        # Convert observation to per-agent format
    ...
    def step(self, action_dict):
        ...
        # Calcular las recompensas usando la estrategia seleccionada 
        rewards = self.reward_strategy.compute_rewards(self.env)
        
        """ validar el codigo de rewards antes de eliminarlo
        # Calculate rewards
        rewards = []
...
def get_env_config(render_mode=None):
    ...
    return {
        ...
        "reward_strategy": "progress",  # Estrategia por defecto para la recompensa / reward
    }
...
def setup_policies_and_config():
    ...
    """
                .training(
                  initial_alpha=1.001,  # Actualizado del ejemplo
                  actor_lr=2e-4,  # Valores más conservadores del ejemplo
                  critic_lr=8e-4,
                  alpha_lr=9e-4,
                  lr=None,
                  target_entropy="auto",
                  n_step=(1, 5),  # Agregado del ejemplo
                  tau=0.005,
                  ...
                  num_steps_sampled_before_learning_starts=10000,  # Aumentado para mayor estabilidad
    )
        .rl_module(
