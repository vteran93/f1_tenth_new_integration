...
def run_collision_analysis_victor_env():
    ...
    for episode in range(20):  # Start with fewer episodes for testing
        ...
        while not done and step_count < 200:
            ...
            step_count += 1
            episode_reward += reward[0] if isinstance(reward, (list, tuple)) else reward  # Handle multi-agent reward

            current_reward = reward[0] if isinstance(reward, (list, tuple)) else reward

            # Check for collision by looking at reward spike
            # Large negative reward indicates collision                print(f"🚨 COLLISION DETECTED - Episode {episode}, Step {step_count}")
            if current_reward < -4.0:
