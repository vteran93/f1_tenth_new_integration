...
class MultiAgentF110(MultiAgentEnv):
    def __init__(self, config):
        # Inicializar agentes antes de super().__init__()
        env_config = config["environment"]
        self._agents = [f"agent_{i}" for i in range(env_config["num_agents"])]
        ...
        render_mode = env_config.get("render_mode") if env_config.get("render_mode") else ("human" if config["mode"] == "eval" else None)
        ...
        print(f"Render training: {self.render_training}, Render interval: {self.render_interval}, Mode: {config['mode']}")
    ...
    def step(self, action_dict):
        ...
        if terminated or truncated:
            ...
            self.episode_rewards = {agent_id: 0.0 for agent_id in self._agents}
        if self.render_training or (config["mode"] == "eval" and self.env.render_mode == "human"):
            self.env.render()
...
def setup_training(config: Dict):
    ...
    """
    if config["mode"] == "eval":
        checkpoint_path = os.path.abspath(os.path.join("models", "multiagent_run_1750594412", "best"))
    ...
    if config["mode"] == "eval":
        checkpoint_path = os.path.abspath(config["checkpoint_path"])  # Usa solo el valor de config_test.yaml
        algo = algo_config.build(logger_creator=custom_log_creator(log_dir))
        algo.restore(checkpoint_path)
        print(f"Restored checkpoint from {checkpoint_path}")
    else:
