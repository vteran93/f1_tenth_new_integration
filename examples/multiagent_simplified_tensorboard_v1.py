...
class MultiAgentF110(MultiAgentEnv):
    def __init__(self, config):
        ...
        self.step_count = 0
        self.episode_step_count = 0  # Nueva variable para rastrear pasos por episodio
        self.agents = [f"agent_{i}" for i in range(env_config["num_agents"])]
        render_mode = env_config.get("render_mode") if not self.render_training else "human"
        self.env = F110Env(config=env_config or {}, render_mode=render_mode)
        self.num_beams = env_config["num_beams"]
        ...
        self.reward_fn = {}
        for agent_id in self.agents:
            strategy = reward_config["agents"][agent_id]["strategy"]
            reward_params = reward_config["agents"][agent_id][strategy]
        ...
        self.last_components = {agent_id: {} for agent_id in self.agents}
        self.episode_rewards = {agent_id: 0.0 for agent_id in self.agents}
        print(f"Action space: {self._action_space}")
        print(f"Observation space: {self._observation_space}")
        print(f"Render training: {self.render_training}, Render interval: {self.render_interval}")
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.episode_rewards = {agent_id: 0.0 for agent_id in self.agents}
        self.step_count = 0
        self.episode_step_count = 0  # Reiniciar pasos por episodio
        obs_dict = {}
        ...
        for agent_id in self.agents:
            positions = [(obs['poses_x'][i], obs['poses_y'][i]) for i in range(len(self.agents))]
            self.reward_fn[agent_id].reset(positions)
        if self.render_training:
            print("Rendering reset")
            self.env.render()
    ...
    def step(self, action_dict):
        ...
        actions = np.zeros((len(self.agents), 2), dtype=np.float32)
        for i, agent_id in enumerate(self.agents):
        ...
        for i, agent_id in enumerate(self.agents):
            ...
            self.episode_rewards[agent_id] += reward  # Acumular recompensa por episodio
            print(f"Agent {agent_id} reward: {reward:.2f}, components: {components}")
        terminated_dict = {agent_id: terminated for agent_id in self.agents}
        terminated_dict["__all__"] = terminated
        truncated_dict = {agent_id: truncated for agent_id in self.agents}
        ...
        if terminated or truncated:
            ...
            self.episode_rewards = {agent_id: 0.0 for agent_id in self.agents}  # Reiniciar al terminar
        if self.render_training and self.step_count % self.render_interval == 0:
            print(f"Rendering step {self.step_count}")
            self.env.render()
...
def setup_training(config: Dict):
    ...
    register_env("f110_multi", lambda env_config: MultiAgentF110(config))
