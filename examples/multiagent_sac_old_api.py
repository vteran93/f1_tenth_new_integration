...
Uses the old API stack for better stability.
"""
...
def setup_policies_and_config():
    ...
    # Configure SAC with OLD API stack for stability
    config = (SACConfig()
              .environment("f1tenth_multi", env_config=get_env_config())
              .framework("torch")
              .api_stack(enable_rl_module_and_learner=False, enable_env_runner_and_connector_v2=False)  # Use OLD API
              .env_runners(num_env_runners=0)  # Use new env_runners API
              .training(
                  initial_alpha=1.0,
                  actor_lr=3e-4,
                  critic_lr=3e-4,
...
def setup_training():
    ...
    try:
        while True:
            ...
            # Use the correct key for timesteps in old API
            timesteps_total = result.get('timesteps_total', result.get('episode_len_mean', 0))
            training_iteration = result.get('training_iteration', 0)

            print(f"Training iteration: {training_iteration}, Timesteps: {timesteps_total}")

            if timesteps_total > 0 and int(timesteps_total) % SAVE_EVERY == 0:
